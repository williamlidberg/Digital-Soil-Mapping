...
PENNOCK = 'D:/WilliamLidberg/FeatureExtraction/PennockLandform/'

#Set number of threads to use. The SGU computer has 10 cores and 20 threads. 
maxThreads = 8      #Change this to adjust the number of cores used
...
class workerThread(threading.Thread):
    ...
    def run(self):
        ...
        #Output files
        outputPENNOCK = PENNOCK + self.file

        
        #Arguments

        args1 = ['--input="' + inputdem + '"',  '--output="' + outputPENNOCK + '"']

        try:
            wbt.run_tool('PennockLandformClass', args1, callback)
