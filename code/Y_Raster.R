...
stack_listoutname <- sub(stack_list,pattern=".tif",replacement="_Y")

#Output directory
outpath <- "D:/MLWAM_Production/Rasters/Original/Y__Coordinates/"
...
foreach(i=1:length(stack_list), .packages=c("foreach","doParallel","raster","sp","rgdal")) %dopar% {
  ...
  ymatrix <- matrix(yFromCell(rasterdf,c(1:1562500)),nrow=1250,byrow=TRUE)
