...
MULTISCALEROUGHNESS = 'D:/WilliamLidberg/FeatureExtraction/MultiscaleRoughnessMagnitude/'
SCALE = 'D:/WilliamLidberg/FeatureExtraction/MultiscaleRoughnessSCALE/'
...
class workerThread(threading.Thread):
    ...
    def run(self):
        ...
        #Output files
        outputMED = MULTISCALEROUGHNESS + self.file.replace('.dep', '.tif')
        outputSCALE = SCALE + self.file.replace('.dep', '.tif')

        
        #Arguments

        args1 = ['--dem=' + inputdem, '--out_mag=' + outputMED, '--out_scale=' + outputSCALE, '--min_scale=1', '--max_scale=10', '--step=2']

        try:
            wbt.run_tool('MultiscaleRoughness', args1, callback)
