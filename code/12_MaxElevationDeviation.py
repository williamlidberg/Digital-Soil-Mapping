...
DEMS = 'C:/data/lokala_exe/DEM/'
MAXELEVATIONDEVIATION = 'D:/WilliamLidberg/FeatureExtraction/MaxElevationDeviation/'
SCALE = 'D:/WilliamLidberg/FeatureExtraction/MaxElevationDeviationSCALE/'
...
class workerThread(threading.Thread):
    ...
    def run(self):
        ...
        #Input file
        inputdem = DEMS + self.file
        
        #Output files
        outputMED = MAXELEVATIONDEVIATION + self.file.replace('.dep', '.tif')
        outputSCALE = SCALE + self.file.replace('.dep', '.tif')

        
        #Arguments

        args1 = ['--dem=' + inputdem, '--out_mag=' + outputMED, '--out_scale=' + outputSCALE, '--min_scale=50', '--max_scale=500', '--step=50']

        try:
            wbt.run_tool('MaxElevationDeviation', args1, callback)
